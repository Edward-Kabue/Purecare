...
include_once(_PS_MODULE_DIR_.'labvegamenu/classes/LabVegamenuClass.php');
include_once(_PS_MODULE_DIR_.'labvegamenu/classes/LabVegamenuRowClass.php');
include_once(_PS_MODULE_DIR_.'labvegamenu/classes/LabVegamenuColumnClass.php');
include_once(_PS_MODULE_DIR_.'labvegamenu/classes/LabVegamenuItemClass.php');
include_once(_PS_MODULE_DIR_.'labvegamenu/sql/SampleDataVMenu.php');

class LabVegamenu extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->getTranslator()->trans('LAB Vegamenu', array(), 'Modules.LabVegamenu.Admin');
		$this->description = $this->getTranslator()->trans('Show Megamenus on your site', array(), 'Modules.LabVegamenu.Admin');
	...
	public function getContent()
	{
		...
		elseif ((Tools::isSubmit('buildMenu') || Tools::isSubmit('submitRow') || Tools::isSubmit('delete_row')) && Tools::isSubmit('id_labvegamenu') || Tools::isSubmit('changeStatusRow') || Tools::isSubmit('submitCol') || Tools::isSubmit('delete_col') || Tools::isSubmit('submitSubItem') || Tools::isSubmit('delete_submenu_item') || Tools::isSubmit('changeStatusSubItem'))
		{
			$this->_postProcess();
		...
		elseif (Tools::isSubmit('addMenu') || (Tools::isSubmit('id_labvegamenu') && $this->menuExists(Tools::getValue('id_labvegamenu'))))
	...
	public function renderList()
	{
		...
		{
			$info_menus[$key]['status'] = $this->displayStatus($info_menu['id_labvegamenu'], $info_menu['active']);
	...
	public function getMenuInfo($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT pc.*, pl.*
			FROM '._DB_PREFIX_.'labvegamenu_shop pc
			LEFT JOIN '._DB_PREFIX_.'labvegamenu_lang pl ON (pc.id_labvegamenu = pl.id_labvegamenu AND pc.`id_shop` = pl.`id_shop`)
			WHERE pl.id_shop = '.$id_shop.' AND pl.id_lang = '.$id_lang.($active ? ' AND pc.`active` = 1' : ' ').' ORDER BY pc.position ASC, pc.id_labvegamenu ASC'
		);
	}
	
	public function displayStatus($id_labvegamenu, $active)
	{
...
/* public function _displayAdvertising()
{
	...
	protected function _postProcess()
	{
		...
		{
			$this->_clearCache('labvegamenu.tpl');
			if (Tools::getValue('id_labvegamenu'))
			{
				$menu_item = new LabVegamenuClass((int)Tools::getValue('id_labvegamenu'));
				$pos_arr = $menu_item->getPositionByMenu((int)Tools::getValue('id_labvegamenu'));
				$menu_item->position = $pos_arr['position'];
				if (!Validate::isLoadedObject($menu_item))
				{
					$this->html .= $this->displayError($this->l('Invalid id_labvegamenu'));
					return false;
				}
			}
			else
			{
				$menu_item = new LabVegamenuClass();
		...
		elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_labvegamenu'))
		{
			$this->_clearCache('labvegamenu.tpl');
			$menu = new LabVegamenuClass((int)Tools::getValue('id_labvegamenu'));
			if ($menu->active == 0)
				$menu->active = 1;
			else
				$menu->active = 0;
			
			$pos_arr = $menu->getPositionByMenu((int)Tools::getValue('id_labvegamenu'));
		...
		elseif (Tools::isSubmit('removeIcon') && Tools::isSubmit('id_labvegamenu'))
		{
			$this->_clearCache('labvegamenu.tpl');
			$menu = new LabVegamenuClass((int)Tools::getValue('id_labvegamenu'));
			$icon = $menu->icon;
			if (preg_match('/sample/', $icon) === 0)
				if ($icon && file_exists(_PS_MODULE_DIR_.'labvegamenu/views/img/icons/'.$icon))
					@unlink(_PS_MODULE_DIR_.'labvegamenu/views/img/icons/'.$icon);
			$menu->icon = '';
			$menu->update();
		}
		elseif (Tools::isSubmit('submitRow'))
		{
			...
			{
				$row_item = new LabVegamenuRowClass(Tools::getValue('id_row'));
				...
				$row_item = new LabVegamenuRowClass();
			$row_item->active = (int)Tools::getValue('active');
			$row_item->id_labvegamenu = (int)Tools::getValue('id_labvegamenu');
		...
		{
			$this->_clearCache('labvegamenu.tpl');
			$menu_item = new LabVegamenuClass((int)Tools::getValue('delete_id_menu'));
			$res = $menu_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_labvegamenu='.Tools::getValue('id_labvegamenu'));
		}
		elseif (Tools::isSubmit('delete_row'))
		{
			$this->_clearCache('labvegamenu.tpl');
			$row_item = new LabVegamenuRowClass((int)Tools::getValue('id_row'));
			$res = $row_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_labvegamenu='.Tools::getValue('id_labvegamenu').'&buildMenu=1');
		}
		elseif (Tools::isSubmit('changeStatusRow') && Tools::isSubmit('id_row'))
		{
			...
			$this->_clearCache('labvegamenu.tpl');
			$id_shop = (int)$this->context->shop->id;
			$row = new LabVegamenuRowClass((int)Tools::getValue('id_row'), null, $id_shop);
			if ($row->active == 0)
				$row->active = 1;
			else
				$row->active = 0;
		...
		elseif (Tools::isSubmit('submitCol'))
		{
			...
			{
				$col_item = new LabVegamenuColumnClass(Tools::getValue('id_column'));
			...
			{
				$col_item = new LabVegamenuColumnClass();
		...
		{
			$this->_clearCache('labvegamenu.tpl');
			$col_item = new LabVegamenuColumnClass((int)Tools::getValue('id_column'));
			$res = $col_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
		...
		elseif (Tools::isSubmit('submitSubItem'))
		{
			...
			{
				$sub_menu_item = new LabVegamenuItemClass(Tools::getValue('id_item'));
			...
			{
				$sub_menu_item = new LabVegamenuItemClass();
		...
		{
			$this->_clearCache('labvegamenu.tpl');
			$submenu_item = new LabVegamenuItemClass((int)Tools::getValue('id_item'));
	...
	public function renderAddForm()
	{
		...
		$id_labvegamenu = Tools::getValue('id_labvegamenu');
		if (isset($id_labvegamenu))
			$menu = new LabVegamenuClass($id_labvegamenu);
		else
			$menu = new LabVegamenuClass();
	...
	public function renderAddMeniItem()
	{
		...
		$fields_form = array(
			'form' => array(
				...
				'buttons' => array(
					array(
					'href' => AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&id_labvegamenu='.$id_labvegamenu.'&buildMenu=1',
					'title' => $this->l('Cancel'),
		...
		if (Tools::isSubmit('id_item'))
		{
			...
			$subMenu_item = new LabVegamenuItemClass(Tools::getValue('id_item'));
		...
		$helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_labvegamenu='.$id_labvegamenu.'&buildMenu=1';
	...
	public function getAddFieldsValuesLink()
	{
		...
		if (Tools::isSubmit('id_item'))
		{
			$menu_item = new LabVegamenuItemClass((int)Tools::getValue('id_item'));
		...
		{
			$menu_item = new LabVegamenuItemClass();
	...
	public function renderAddRow()
	{
		...
		$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_labvegamenu');
	...
	public function getAddFieldsValues()
	{
		...
		if (Tools::isSubmit('id_row'))
		{
			$row = new LabVegamenuRowClass((int)Tools::getValue('id_row'));
			$fields['id_row'] = (int)Tools::getValue('id_row', $row->id);
			$fields['active'] = Tools::getValue('active', $row->active);
			$fields['id_labvegamenu'] = Tools::getValue('id_labvegamenu', $row->id_labvegamenu);
			$fields['class'] = Tools::getValue('class', $row->class);
		}
		else
		{
			$row = new LabVegamenuRowClass();
			$fields['active'] = Tools::getValue('active', 1);
			$fields['id_labvegamenu'] = Tools::getValue('id_labvegamenu', 1);
	...
	public function getAddFieldsValuesCol()
	{
		...
		if (Tools::isSubmit('id_column'))
		{
			$col = new LabVegamenuColumnClass((int)Tools::getValue('id_column'));
		...
		{
			$col = new LabVegamenuColumnClass();
			$fields['active'] = Tools::getValue('active', 1);
	...
	public function renderBuildMenu()
	{
		...
		$this->context->controller->addJS($this->_path.'views/js/back.js');
		$this->context->controller->addCSS($this->_path.'views/css/back.css');
		$id_labvegamenu = Tools::getValue('id_labvegamenu', 1);
		$info_rows = $this->getRowInfo($id_labvegamenu);
		
		foreach ($info_rows as $key => $info_row)
		{
			$info_rows[$key]['status'] = $this->displayStatusRow($id_labvegamenu, $info_row['id_row'], $info_row['active']);
	...
	public function getRowInfo($id_menu, $active = null)
	{
		...
		$row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'labvegamenu_row_shop mr
			WHERE mr.`id_shop` = '.$id_shop.' AND mr.id_labvegamenu = '.$id_menu.($active ? ' AND mr.`active` = 1' : ' ')
		);
	...
	public function getColInfo($id_row, $active = null, $is_backend = true)
	{
		...
		$cols_result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'labvegamenu_column_shop mc
		...
		if (is_array($col_infos) && count($col_infos) > 0)
			foreach ($col_infos as $key => $col_info)
			{
				$sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					SELECT smi.*,smil.*
					FROM '._DB_PREFIX_.'labvegamenu_item_shop smi
					LEFT JOIN '._DB_PREFIX_.'labvegamenu_item_lang smil ON (smi.id_item = smil.id_item AND smi.id_shop = smil.id_shop)
				...
				if (is_array($sub_menu_infos) && count($sub_menu_infos) > 0)
				{
					...
					{
						$id_labvegamenu = Tools::getValue('id_labvegamenu', 1);
						if (isset($id_labvegamenu) && $id_labvegamenu > 0 && $is_backend)
							$sub_menu_infos[$key1]['status'] = $this->displayStatusSubItem($id_labvegamenu, $sub_menu_info['id_item'], $sub_menu_info['active']);
	...
	public function getSubMenu($id_labvegamenu)
	{
		$info_rows = array();
		if (is_array($this->getRowInfo($id_labvegamenu, true)) && count($this->getRowInfo($id_labvegamenu, true)) > 0)
		{
			$info_rows = $this->getRowInfo($id_labvegamenu, true);
			foreach ($info_rows as $key => $info_row)
	...
	{
		$req = 'SELECT lab.`id_labvegamenu` as id_labvegamenu
				FROM `'._DB_PREFIX_.'labvegamenu` lab
				WHERE lab.`id_labvegamenu` = '.(int)$id;
	...
	public function hookHeader()
	{
		...
		$this->context->controller->addCSS($this->_path.'views/css/labvegamenu.css');
	}
	 ...
	 public function getWidgetVariables($hookName = null, array $configuration = [])
    {
		...
		if (!$this->isCached('labvegamenu.tpl', $this->getCacheId('labvegamenu')))
		{
			$menu_obj = new LabVegamenuClass();
			...
			foreach ($menus as $menu)
			{
				...
				else
				{
					...
					$sub_menu = $this->getSubMenu($menu['id_labvegamenu']);
	...
	public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
	{
		...
		{
			$this->respMenu .= '<li class="laberCart '.$class.'">';
			if ( $icon )
				$laberIcon ='laberIcon';
			$this->respMenu .= '<a href="'.$cat_link.'" class="'.$laberIcon.'">';
			if ($type_icon == 1 && $icon != '')
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_shop (`id_labvegamenu`, `id_shop`, `type_link`, `dropdown`, `type_icon`, `icon`, `align_sub`, `width_sub`, `class`, `active`)
			SELECT id_labvegamenu, '.(int)$params['new_id_shop'].', type_link, dropdown, type_icon, icon, align_sub, width_sub, class, active
			FROM '._DB_PREFIX_.'labvegamenu_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_lang (`id_labvegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_labvegamenu, '.(int)$params['new_id_shop'].', id_lang, title, link, subtitle
			FROM '._DB_PREFIX_.'labvegamenu_lang
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_row_shop (`id_row`, `id_labvegamenu`, `id_shop`, `class`, `active`)
			SELECT id_row, id_labvegamenu, '.(int)$params['new_id_shop'].', class, active
			FROM '._DB_PREFIX_.'labvegamenu_row_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_column_shop (`id_column`, `id_row`, `id_shop`, `width`, `class`, `active`)
			SELECT id_column, id_row, '.(int)$params['new_id_shop'].', width, class, active
			FROM '._DB_PREFIX_.'labvegamenu_column_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_item_shop (`id_item`, `id_column`, `id_shop`, `type_link`, `type_item`, `id_product`, `active`)
			SELECT id_item, id_column, '.(int)$params['new_id_shop'].', type_link, type_item, id_product, active
			FROM '._DB_PREFIX_.'labvegamenu_item_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_item_lang (`id_item`, `id_shop`, `id_lang`, `title`, `link`, `text`)
			SELECT id_item, '.(int)$params['new_id_shop'].', id_lang, title, link, text
			FROM '._DB_PREFIX_.'labvegamenu_item_lang
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
	}
	public function hookActionObjectLanguageAddAfter($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'labvegamenu_lang (`id_labvegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_labvegamenu, id_shop, '.(int)$params['object']->id.', title, link, subtitle
			FROM '._DB_PREFIX_.'labvegamenu_lang
